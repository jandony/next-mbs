...
import browserStates from '@/app/demoData/table-data.json'
...
const BrowserState = React.memo(() => {
    ...
    if (
        browserState !== null &&
        browserState[browserTab as keyof sellingData]
    ) {
        browserState[browserTab as keyof sellingData].map(
            (value: Value, index: number) => {
                const { key, name, img, session, bounceRate, cte, goalRate } =
                    value
                return browserData.push({
                    key: index + 1,
                    name: (
                        <div className="flex items-center">
                            <div className="mr-2.5 w-8 h-8">
                                <Image
                                    className="w-8 h-8"
                                    src={`/img/browser/${img}`}
                                    width="32"
    ...
    const browserColumns = [
        {
            ...
            key: 'name',
            className:
                'pr-4 text-light dark:text-white/60 text-[12px] py-2.5 last:text-end border-none uppercase before:hidden rounded-l-4',
        },
        {
            title: 'Session',
            dataIndex: 'session',
            key: 'session',
            className:
                'pr-4 text-light dark:text-white/60 text-[12px] py-2.5 last:text-end border-none uppercase before:hidden',
        },
        {
            title: 'Bounce rate',
        ...
        },
        {
            ...
            key: 'goalRate',
            className:
                'pr-4 text-light dark:text-white/60 text-[12px] py-2.5 last:text-end border-none uppercase before:hidden rounded-r-4',
        },
    ]
